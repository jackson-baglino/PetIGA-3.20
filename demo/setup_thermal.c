...
static PetscErrorCode ComputeCircleIceField(AppCtx *user)
{
  ...
  /* Create a PETSc Vec to hold the ice field at nodal points */
  Vec ice;
  ...
  while (IGANextElement(user->iga, element)) {
	...
	while (IGAElementNextPoint(element, point)) {
	  ...
	  // For debugging
	  if (idx > total) {
		PetscPrintf(PETSC_COMM_WORLD, "There are %d points in ice vec. Originaly set to %d.\n", idx, total);
	  }
	}
   ...
   Purpose:  Determines the initialization mode and calls the
			 appropriate helper function:
			   ...
			   - Otherwise: Assumes user->init_mode is a file path,
				 ...
				 InterpolateIceField, and stores the result in a Vec.
   Parameters:
...
PetscFunctionBegin;
  ...
  if (strcmp(user->init_mode, "circle") == 0) {
	...
	/* File mode: read raw data and use it directly */
	...
	/* Create a PETSc Vec and copy raw data into it */
	Vec ice;
	ierr = VecCreate(PETSC_COMM_WORLD, &ice); CHKERRQ(ierr);
	ierr = VecSetSizes(ice, PETSC_DECIDE, nRaw); CHKERRQ(ierr);
	ierr = VecSetFromOptions(ice); CHKERRQ(ierr);
	PetscScalar *iceArray;
	ierr = VecGetArray(ice, &iceArray); CHKERRQ(ierr);
	for (PetscInt i = 0; i < nRaw; i++) {
	  iceArray[i] = rawField[i];
	}
	ierr = VecRestoreArray(ice, &iceArray); CHKERRQ(ierr);
	ierr = PetscFree(rawField); CHKERRQ(ierr);

	PetscPrintf(PETSC_COMM_WORLD, "File mode: ice field loaded with %d points.\n", nRaw);
	user->ice = ice;
  }
