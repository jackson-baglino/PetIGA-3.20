...
typedef struct {
  IGA iga;  // Isogeometric Analysis context (for solving PDEs)

  // Model Parameters
  PetscReal eps;  // Interface width
  PetscReal mob_sub, mav, Etai, Etam, Etaa, alph_sub, Lambd, beta_sub0, d0_sub0;
  PetscReal air_lim, xi_v, xi_T;  // Limits and coefficients

  // Thermophysical Properties
  PetscReal thcond_ice, thcond_met, thcond_air; // Thermal conductivities
  PetscReal cp_ice, cp_met, cp_air;             // Specific heat capacities
  PetscReal rho_ice, rho_met, rho_air;          // Densities
  PetscReal dif_vap, lat_sub, diff_sub;         // Diffusion coefficients and latent heat

  // Temperature and Initial Conditions
  PetscReal T_melt;        // Melting temperature
  PetscReal temp0, hum0;   // Initial temperature and humidity
  PetscReal grad_temp0[3]; // Initial temperature gradient (x, y, z)

  // Domain Size and Discretization
  PetscReal Lx, Ly, Lz;    // Domain dimensions
  PetscReal Nx, Ny, Nz;    // Number of grid points in each dimension

  // Grain and Sediment Properties
  PetscReal RCice, RCsed;        // Characteristic radii for ice and sediment
  PetscReal RCice_dev, RCsed_dev; // Deviation in characteristic radii
  PetscReal cent[3][200], radius[200]; // Centers and radii of ice grains
  PetscReal centsed[3][200], radiussed[200]; // Centers and radii of sediment grains

  // Boundary/Interface Normal Vectors
  PetscReal norm0_0, norm0_1, norm0_2; // Normal vector components

  // Flags and Control Variables
  PetscInt flag_it0;   // Flag for initial time step
  PetscInt flag_tIC;   // Flag for initial condition type
  PetscInt flag_xiT;   // Flag for temperature-dependent properties
  PetscInt flag_Tdep;  // Flag for temperature dependence
  PetscInt outp;       // Output control flag
  PetscInt nsteps_IC;  // Number of initial condition steps

  // Problem-Specific Parameters
  PetscInt p, C;       // Some problem parameters
  PetscInt dim;        // Spatial dimension
  PetscInt periodic;   // Flag for periodic boundary conditions

  // Time Control Parameters
  PetscReal t_out;     // Output time
  PetscReal t_interv;  // Time interval for output
  PetscReal t_IC;      // Initial condition application time

  // Active Components Counters
  PetscInt NCice, NCsed;      // Number of ice and sediment components
  PetscInt n_act, n_actsed;   // Number of active ice and sediment components

  // Dynamic Variables
  PetscReal *Phi_sed;  // Phase field for sediment
  PetscReal *alph;     // Alpha parameter array
  PetscReal *mob;      // Mobility array

  // Environmental Conditions
  PetscReal rho_rhovs; // Vapor density
  PetscReal Patm;      // Atmospheric pressure

  // Input Handling
  PetscInt readFlag;   // Flag indicating whether to read input data
  ...
  if(pnt->atboundary){
    ...
    for(a=0; a<nen; a++) {
      ...
      if(user->flag_tIC==1){
        ...
        R_ice  = N0[a]*ice_t; 
        for(l=0;l<dim;l++) R_ice += 0; // 3.0*mob*eps*(N1[a][l]*grad_ice[l]);
        R_ice += 0; // N0[a]*mob*3.0/eps/ETA*((Etam+Etaa)*fice - Etaa*fmet - Etam*fair);
        R_ice -= 0; // N0[a]*alph_sub*ice*ice*air*air*(rhov-rhoI_vs)/rho_ice;        
        ...
        if(air>air_lim){
          ...
          for(l=0;l<dim;l++) R_vap += 0; // xi_v*difvap*air*(N1[a][l]*grad_rhov[l] );
        } else {
          R_vap += N0[a]*air_lim*rhov_t;
          for(l=0;l<dim;l++) R_vap += 0; // xi_v*difvap*air_lim*(N1[a][l]*grad_rhov[l] );
        }
        R_vap -=  0; // xi_v*N0[a]*rhoSE*air_t;        
    ...
    for(a=0; a<nen; a++) {
      for(b=0; b<nen; b++) {
        ...
        //ice
          ...
          for(l=0;l<dim;l++) J[a][0][b][0] += 0; // 3.0*mob*eps*(N1[a][l]*N1[b][l]);

          J[a][0][b][0] += 0; // N0[a]*mob*3.0/eps/ETA*((Etam+Etaa)*fice_ice - Etaa*fmet_ice - Etam*fair_ice)*N0[b];
          J[a][0][b][0] -= 0; // N0[a]*alph_sub*2.0*ice*N0[b]*air*air*(rhov-rhoI_vs)/rho_ice;
          J[a][0][b][0] += 0; // N0[a]*alph_sub*ice*ice*2.0*air*N0[b]*(rhov-rhoI_vs)/rho_ice;
          J[a][0][b][1] += 0; // N0[a]*alph_sub*ice*ice*air*air*drhoI_vs*N0[b]/rho_ice;
          J[a][0][b][2] -= 0; // N0[a]*alph_sub*ice*ice*air*air*N0[b]/rho_ice;
        ...
        //vapor density
          J[a][2][b][0] -= 0; // N0[a]*rhov*shift*N0[b];
          J[a][2][b][2] += 0; // N0[a]*N0[b]*air_t;
          if(air>air_lim){
            J[a][2][b][0] -= 0; //N0[a]*N0[b]*rhov_t;
            J[a][2][b][2] += N0[a]*air*shift*N0[b];
            for(l=0;l<dim;l++) J[a][2][b][0] -= 0; // xi_v*difvap*N0[b]*(N1[a][l]*grad_rhov[l]);
            for(l=0;l<dim;l++) J[a][2][b][1] += 0; // xi_v*d_difvap*N0[b]*air*(N1[a][l]*grad_rhov[l]);
            for(l=0;l<dim;l++) J[a][2][b][2] += 0; // xi_v*difvap*air*(N1[a][l]*N1[b][l]);        
          } else {
            J[a][2][b][2] += 0; // N0[a]*air_lim*shift*N0[b];
            for(l=0;l<dim;l++) J[a][2][b][1] += 0; // xi_v*d_difvap*N0[b]*air_lim*(N1[a][l]*grad_rhov[l] );
            for(l=0;l<dim;l++) J[a][2][b][2] += 0; // xi_v*difvap*air_lim*(N1[a][l]*N1[b][l]);
          }
          J[a][2][b][0] += 0; // xi_v*N0[a]*rhoSE*shift*N0[b];
