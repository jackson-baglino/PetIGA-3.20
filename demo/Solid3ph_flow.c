...
//------print information  
  ...
  if(step % 2 == 0) {
    ...
    // char dir[256]; sprintf(dir,"/Users/jacksonbaglino/SimulationResults/junning");
    char  filedata[256];
...
/*  if(step==0) {
    ...
    //const char *env = "folder"; char *dir; dir = getenv(env);
    char dir[256]; sprintf(dir,"/home/junning/Simulation/solidif_results");
    char  fileiga[256];
  ...
  if(print == 1) {
    if(user->periodic==0) {  //------------ Periodic BC does not support VecStrideScatter/Gather, thus we need to save vector SedV in a separate file 
      ...
      if(user->flag_flux==1 && step>0){
        ...
        sprintf(filenamere,"%s/ksp_pres%d.dat",dir,user->last_step_out);
      ...
      if(user->flag_flux==1) {
        ...
        sprintf(filenamepres,"%s/ksp_pres%d.dat",dir,step);
      ...
      //const char *env = "folder"; char *dir; dir = getenv(env); for non-periodic files
      const char *env = "folder"; char *dir; dir = getenv(env);
