...
- **Cost model was ~15–20× high.** The pilot's ~6650 steps at −20 °C came from
  its leg 1 running at the old flat -dtmax 200 (97% of steps at the cap); leg 2
  on the derived 1.09·τ_sub did days 15–30 in 165 steps. rev64 (derived dtmax
  throughout) did 30 d in 371 steps, 2 h 19 min at 615 ranks, ~$17. CAMPAIGN.md
  stage 2/3 costs rewritten; −10 °C no longer needs restart legs.
- New `studies/keff_sintering/batch2_T_warm.txt`: φ 0.325, seeds 1/3/4 ×
  T −20/−15/−10, k_eff every step (-keff_freq 1), -t_out_log 150. ~$150 total.
  −20 re-run rather than reused: pilot mixes two dt caps and its SSA slope
  jumps at the switch; the pairing with pilot seeds is a free dt check.
- Generated the −15 °C opts (seeds 1/3/4 + snow_T-15_h1.00_30d) with
  generate_study_opts.py; −10/−20 regenerated byte-identical.
- `submit_batch.sh --out-root DIR`: batch parent under DIR/enceladus_DSM/
  instead of $SCRATCH (fails if DIR is not writable). For paper runs.
- Porosity ceiling answered: 0.35 with margin (all seeds percolate), 0.40
  formal ceiling, T-independent.
