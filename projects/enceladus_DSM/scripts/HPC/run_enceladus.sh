...
#   $4.. : extra options forwarded verbatim to the enceladus_dsm executable,
#          appended after the three -options_file flags so they override
#          anything set in solver.opts/geometry/experiment opts
#          (e.g. -d0_GT 1.0e-8). Populated by submit_enceladus.sh's `--`
#          separator.
###############################################################################
...
#   $SCRATCH/enceladus_DSM/<geometry>/<timestamp>_<experiment>[_<tag>][_job<id>]
#
...
create_folder() {
    ...
    local base_dir
    if [[ -d "${SCRATCH:-}" ]]; then
        base_dir="$SCRATCH/enceladus_DSM"
    else
...
echo ""
echo "========================================================================="
echo "  Enceladus DSM simulation workflow (HPC)"
echo "  Project root : $PROJECT_ROOT"
