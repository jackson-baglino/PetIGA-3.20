#!/usr/bin/env bash
# =============================================================================
# submit_enceladus.sh — compute optimal MPI ranks and submit via sbatch
#
# Usage (run from project root):
#   ./scripts/HPC/submit_enceladus.sh <geometry> <experiment> [tag] \
...
#   - after `--`: forwarded verbatim to the enceladus_dsm executable itself
#     (appended after the three -options_file flags, so they override
#     anything set in solver.opts/geometry/experiment opts files).
#
# Examples:
#   ./scripts/HPC/submit_enceladus.sh multigrain_2D_L100um_eps0.37um_test base_T-20_h0.95_1d
#
#   ./scripts/HPC/submit_enceladus.sh multigrain_2D_L100um_eps0.37um_test base_T-20_h0.95_1d p2_run \
#       --time=0-12:00:00 --partition=expansion
#
#   ./scripts/HPC/submit_enceladus.sh twograins_2D_L100um_eps0.37um_2grain base_T-20_h0.95_1d \
...
# resolve_opts now lives in scripts/lib/opts.sh (sourced below).
...
# resolve_opts/require_opts. Sourced HERE, not next to alloc.sh further down:
# the geometry/experiment lookup happens well before that point.
...
# it are extra options forwarded to the enceladus_dsm executable.
...
# runner (run_enceladus.sh) clamps its own rank count to the SLURM
...
# run_enceladus.sh. Formula: ceil(dof * Nx * Ny * Nz / TARGET_DOFS_PER_CORE)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Submit — --ntasks/--nodes override the #SBATCH defaults in run_enceladus.sh
# ---------------------------------------------------------------------------
