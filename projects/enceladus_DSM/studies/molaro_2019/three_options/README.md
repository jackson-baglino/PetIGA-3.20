...
> measured with the old linear-in-φ sub-cell crossing, which carried a ±0.06 µm
> sub-cell artefact (see `../three_options_round2/README.md`). The local copy of
> the batch was deleted, but **it survives on the HPC** — re-run
> `postprocess/run_batch_measure.sh` on it there and update `summary.csv`, which
> puts both ends of every family line in `tuning_ceiling.png` on the same
> convention. Until then, do not quote these widths to better than ~0.1 µm.
