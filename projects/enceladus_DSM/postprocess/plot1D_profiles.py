...
def plot_thermal_overlay(run_dir: str, save_path: str = None,
    ...
    run_label = os.path.basename(run_dir.rstrip("/")) or run_dir
    ax_T.set_title(f"1D Phase-field — thermal fields (all snapshots)\n({run_label})",
                   fontsize=13)
    ax_T.grid(True, alpha=0.3)
...
def plot_derived(run_dir: str, save_path: str = None,
    ...
    run_label = os.path.basename(run_dir.rstrip("/")) or run_dir
    axes[0].set_title(f"1D Phase-field — derived quantities\n({run_label})", fontsize=13)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Plot 1D phase-field field profiles.",
