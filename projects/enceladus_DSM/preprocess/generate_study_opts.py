...
def _dur_token(seconds: float) -> str:
    ...
    for name, size, limit in (("d", 86400, 1e9), ("h", 3600, 48), ("min", 60, 120)):
        q = seconds / size
        if abs(q - round(q)) < 1e-9 and 1 <= round(q) < limit:
            return f"{round(q):g}{name}"
    return (f"{seconds:.3g}s" if seconds < 1000
            else f"{seconds:.2e}".replace("e+0", "e").replace("e+", "e") + "s")


def _humidity_token(h: float) -> str:
    return "h1.00" if h == 1.0 else "h" + ("%.3f" % h).rstrip("0").rstrip(".")
...
def main(argv=None):
    ...
    for T in args.temps:
        ...
        # Naming scheme: <campaign>_T<T>_h<h>_<duration>[_modifiers], one
        # subdirectory per campaign. alpha_c is held fixed across this sweep,
        # so it is documented in the header rather than encoded in the name.
        exp_name = (f"snow_T{T:g}_{_humidity_token(args.humidity)}"
                    f"_{_dur_token(args.t_final)}{sfx}")
        write_experiment(exp_dir / "snow" / f"{exp_name}.opts", T, p0, args)
        for pk in packs:
