...
Usage
-----
Run from the project root (PetIGA-3.20/projects/enceladus_DSM/):
...
def generate_random_bumps(rng, Lx_, n, R_range, height_ratio_range, overlap_range,
                           margin=0.0, span_tol=0.03, max_retries=500):
    """Sequential left-to-right walk (not rejection sampling): place n bumps
    with randomized R/height/overlap so their supports always touch or
    overlap, guaranteeing full coverage of [margin, Lx-margin] with no flat
    gaps -- the opposite of random_bump_channel.py's gap_factor>1 spacing.

    height = R * uniform(*height_ratio_range) ties height to R directly,
    bounding curvature (~height/R^2) instead of letting it vary independently.

    overlap (fraction of R_i+R_j that consecutive centers are pulled
    together by) must stay below ~0.36: above that, two overlapping C-inf
    bumps add constructively and the combined peak exceeds either bump
    alone, violating "no overly large bumps". overlap_range=(0.20,0.32)
    keeps clear of that cliff while still avoiding near-zero dips between
    bumps (verified by Monte Carlo when this was designed: worst-case
    combined height ~2.9% of Ly, far under typical budgets).

    Raises if the random draws can't be rescaled to fit Lx within
    span_tol after max_retries attempts -- widen --overlap-range/--R-range
    or change --n-bumps (see the avg_R/avg_step arithmetic this implies:
    (n-1)*avg_step ~= Lx, avg_step = 2*avg_R*(1-avg_overlap))."""
    assert n <= MAX_SED_GRAINS, f"n_bumps={n} exceeds MAX_SED_GRAINS={MAX_SED_GRAINS}"
    usable_span = Lx_ - 2 * margin
    for _ in range(max_retries):
        R = rng.uniform(*R_range, size=n)
        h = R * rng.uniform(*height_ratio_range, size=n)
        overlap = rng.uniform(*overlap_range, size=n - 1)
        step = (R[:-1] + R[1:]) * (1.0 - overlap)
        total = step.sum()
        if abs(total - usable_span) <= span_tol * usable_span:
            step = step * (usable_span / total)
            centers = np.concatenate([[margin], margin + np.cumsum(step)])
            return list(zip(centers.tolist(), R.tolist(), h.tolist()))
    raise RuntimeError(
        f"could not match span to within {span_tol:.0%} of Lx after "
        f"{max_retries} attempts -- widen --overlap-range/--R-range or "
        "change --n-bumps")


def validate_bump_field(bumps, Lx_, Ly_, max_height_frac_of_Ly=0.20, n_check=20000):
    """Sanity-check the combined bump field before writing any file: no
    near-zero flat gaps, no oversized combined peak."""
    x = np.linspace(0.0, Lx_, n_check)
    y = np.zeros_like(x)
    for center, R, height in bumps:
        y = y + _bump(x, center, R, height)
    assert y.min() > 1e-12 * Ly_, f"flat gap detected, min height={y.min():.3e}"
    assert y.max() < max_height_frac_of_Ly * Ly_, \
        f"combined peak too tall: {y.max():.3e} (> {max_height_frac_of_Ly:.0%} of Ly)"
    dx = x[1] - x[0]
    d2y = np.gradient(np.gradient(y, dx), dx)
    print(f"  bump field check: max height={y.max():.3e} m "
          f"({y.max()/Ly_:.1%} of Ly), max curvature={np.abs(d2y).max():.3e}")
...
def main():
    ...
    parser.add_argument("--Lx", type=float, default=Lx,
                         help=f"domain width [m], default {Lx}")
    parser.add_argument("--Ly", type=float, default=Ly,
                         help=f"domain height [m], default {Ly}")
    ...
    parser.add_argument("--bumps", default=None,
                         help="override SEDIMENT_GRAINS for this invocation only, as "
                              "'cx,R,h;cx,R,h;...' (e.g. a single centered bump: "
    ...
    parser.add_argument("--random-bumps", action="store_true",
                         help="generate a full-coverage random bump list (overrides "
                              ...
                              "hardcoded production list -- see generate_random_bumps()")
    parser.add_argument("--seed", type=int, default=0,
                         help="RNG seed for --random-bumps, default 0")
    parser.add_argument("--n-bumps", type=int, default=20,
                         help="number of bumps for --random-bumps, default 20 "
                              f"(hard-capped at MAX_SED_GRAINS={MAX_SED_GRAINS})")
    parser.add_argument("--R-range", type=float, nargs=2, default=(3.0e-6, 4.2e-6),
                         help="bump half-width range [m] for --random-bumps")
    parser.add_argument("--height-ratio-range", type=float, nargs=2, default=(0.15, 0.28),
                         help="height/R ratio range for --random-bumps (bounds curvature)")
    parser.add_argument("--overlap-range", type=float, nargs=2, default=(0.20, 0.32),
                         help="center-spacing overlap fraction range for --random-bumps "
                              "(must stay below ~0.36 to avoid constructive-interference "
                              "peak inflation -- see generate_random_bumps() docstring)")
    ...
    if args.random_bumps:
        ...
        SEDIMENT_GRAINS = generate_random_bumps(
            rng, Lx, args.n_bumps, tuple(args.R_range),
            tuple(args.height_ratio_range), tuple(args.overlap_range), args.margin)
