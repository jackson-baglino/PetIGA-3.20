"""
2dspine100x8.py — 2D B-spline surface for a symmetric fin/spine cross-section.
...
Mesh refinement:
  u-direction: 100 uniformly-spaced knot insertions (fine along the spine)
  v-direction:   8 uniformly-spaced knot insertions (coarse across the width)

Outputs:
  2dspline.dat  — PetIGA binary mesh file
  2dspline.vtk  — VTK structured grid for visualization

See also: 2dspine200x8.py (identical except 200 u-direction insertions).

Usage:
  python3 2dspine100x8.py
"""
...
# Insert knots uniformly: 100 in u (along spine), 8 in v (across width).
# Interior points only — linspace endpoints [0] and [-1] are excluded.
insert_x0 = linspace(0, 1, 100)[1:-1]
