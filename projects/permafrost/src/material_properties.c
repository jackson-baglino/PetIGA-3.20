                 ...
                 PetscScalar *cond, PetscScalar *dcond_ice)
{
    ...
    if (cond) 
        (*cond) = (cond_ice + cond_air + cond_met) / 3.0; // ice * cond_ice + met * cond_met + air * cond_air;
...
if (cp) 
(*cp) = (cp_ice + cp_air + cp_met) / 3.0; //ice * cp_ice + met * cp_met + air * cp_air;
...
if (rho) 
(*rho) = (rho_ice + rho_air + rho_met) / 3.0; //ice * rho_ice + met * rho_met + air * rho_air;
