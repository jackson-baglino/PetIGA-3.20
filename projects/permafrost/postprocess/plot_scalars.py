...
step and has 4 columns:

    sub_interf/eps   tot_ice   t[s]   step
...
Derived quantities computed here:
  ...
  - Air volume = domain - ice - sediment (requires domain info)
  - Vapor supersaturation (requires vapor + T; not in SSA file — see plot1D_profiles)
...
def load_ssa(path: str) -> np.ndarray:
    """
    Load SSA_evo.dat.  Returns (N, 4) array:
    ...
    # Drop rows that are all-NaN or have fewer than 4 columns
    if data.shape[1] < 4:
        sys.exit(f"Expected ≥ 4 columns in SSA_evo.dat, got {data.shape[1]}")
...
def plot_scalars(data_path: str, time_unit: str = "h", save_path: str = None,
                 title: str = None):
    """
    Plot all scalar quantities from SSA_evo.dat in a 4-panel figure.
    """
    data = load_ssa(data_path)

    intf   = data[:, 0]   # sub_interf / eps
    ice    = data[:, 1]   # tot_ice (length or area)
    t_sec  = data[:, 2]
    steps  = data[:, 3].astype(int)

    t, xlabel = convert_time(t_sec, time_unit)

    # Derived: air = (first value of ice + intf) gives a rough baseline
    # We can compute Δice = ice - ice[0]  (change in ice volume)
    d_ice = ice - ice[0]
    ...
    axes[0].set_title(
        (title or f"Scalar evolution — {os.path.dirname(data_path)}"),
        fontsize=13
    )

    # Panel 2: Change in ice volume (sublimation proxy)
    axes[1].plot(t, d_ice, color="#d62728", lw=2)
    axes[1].axhline(0, color="gray", lw=0.8, ls="--")
    axes[1].set_ylabel(r"$\Delta\int\phi_i\,dV$  (from $t_0$)", fontsize=12)

    # Panel 3: Interface density (proxy for specific surface area)
    axes[2].plot(t, intf,  color="#2ca02c", lw=2)
    axes[2].set_ylabel(r"Interface density  $\Sigma/\varepsilon$", fontsize=12)

    # Panel 4: Step index (adaptive time step diagnostic)
    if len(t) > 1:
        dt_arr = np.diff(t_sec)
        axes[3].semilogy(t[1:], dt_arr, color="#9467bd", lw=1.5)
        axes[3].set_ylabel(r"$\Delta t$  [s]", fontsize=12)
    else:
        axes[3].text(0.5, 0.5, "Single step — no dt info", ha="center", va="center",
                     transform=axes[3].transAxes)

    axes[3].set_xlabel(xlabel, fontsize=12)
