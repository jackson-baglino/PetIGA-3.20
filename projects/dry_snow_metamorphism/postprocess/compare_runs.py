#!/usr/bin/env python3
"""
compare_runs.py  —  Overlay scalar time-series from multiple DSM simulation runs.
...
def compare(run_dirs: list, labels: list = None, time_unit: str = "h",
    ...
    for i, (run_dir, label) in enumerate(zip(run_dirs, labels)):
        ...
        if normalise and ice[0] != 0:
            ...
            ice_label  = r"$\phi_i / \phi_i(t_0)$"
            intf_label = r"$\Sigma / \Sigma(t_0)$"
        else:
            ice_plot  = ice
            intf_plot = intf
            ice_label  = r"$\int \phi_i \, dV$"
            intf_label = r"$\Sigma / \varepsilon$"
...
def compare_1D_profiles(run_dirs: list, labels: list = None,
    ...
    """
    Overlay φ_i(x), T(x), ρ_v(x) from each run at the same snapshot.
    DSM has 3 DOFs: ice, temperature, vapor (no sediment).
    """
    try:
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Compare scalar evolution or 1D profiles across multiple DSM runs."
    )
    ...
    p.add_argument("--snapshot",  default="last",
                   help="Which snapshot to use for profiles: 'first', 'last', or step number")
