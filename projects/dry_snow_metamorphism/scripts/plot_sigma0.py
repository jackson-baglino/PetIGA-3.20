...
def ensure_plot_dir():
    """
    Ensure that the plot output directory exists.
...
def load_sigma0_data():
    """
    Load experimental data for sigma0 as a function of temperature.
    Returns:
        temperature_C (np.ndarray): Temperatures in Celsius.
        sigma0 (np.ndarray): Corresponding sigma0 values.
    """
...
def interpolate_sigma0(temp_C, sigma0):
    """
    Create a linear interpolation function for sigma0 vs temperature.
    Args:
        temp_C (array-like): Temperatures in Celsius.
        sigma0 (array-like): Corresponding sigma0 values.
    Returns:
...
def compute_d0(temp_C):
    """
    Compute the capillary length d0 as a function of temperature.
    Args:
        temp_C (array-like): Temperatures in Celsius.
    Returns:
        np.ndarray: d0 values.
    """
    temp_C = np.asarray(temp_C)
    c_i = RHO_I / M_MOL
...
def compute_alpha_basal(temp_C, sigma0):
    """
    Compute the basal attachment coefficient alpha_basal.
    Args:
        temp_C (array-like): Temperatures in Celsius.
        sigma0 (array-like): sigma0 values.
    Returns:
        np.ndarray: alpha_basal values.
    """
    temp_C = np.asarray(temp_C)
    sigma0 = np.asarray(sigma0)
...
def rhovs_I(temp_C):
    """
    Compute the equilibrium vapor density over ice as a function of temperature.
    Args:
        temp_C (array-like): Temperatures in Celsius.
    Returns:
        np.ndarray: Equilibrium vapor density values.
    """
    temp_C = np.asarray(temp_C)
...
def compute_beta_sub(temp_C, sigma0):
    """
    Compute the attachment coefficient beta_sub for sublimation.
    Args:
        temp_C (array-like): Temperatures in Celsius.
        sigma0 (array-like): sigma0 values.
    Returns:
        np.ndarray: beta_sub values.
    """
    temp_C = np.asarray(temp_C)
    sigma0 = np.asarray(sigma0)
...
def compute_lambda_sub(temp_C, sigma0):
    """
    Compute the lambda_sub parameter for sublimation.
    Args:
        temp_C (array-like): Temperatures in Celsius.
        sigma0 (array-like): sigma0 values.
    Returns:
        np.ndarray: lambda_sub values.
    """
...
def compute_tau_sub(temp_C, sigma0):
    """
    Compute the characteristic time tau_sub for sublimation.
    Args:
        temp_C (array-like): Temperatures in Celsius.
        sigma0 (array-like): sigma0 values.
    Returns:
        np.ndarray: tau_sub values.
    """
...
def compute_mobility(temp_C, sigma0):
    """
    Compute the mobility M0 as a function of temperature and sigma0.
    Args:
        temp_C (array-like): Temperatures in Celsius.
        sigma0 (array-like): sigma0 values.
    Returns:
        np.ndarray: M0 values.
    """
...
def compute_alpha_sub(temp_C, sigma0):
    """
    Compute the attachment factor alpha_sub for sublimation.
    Args:
        temp_C (array-like): Temperatures in Celsius.
        sigma0 (array-like): sigma0 values.
    Returns:
        np.ndarray: alpha_sub values.
    """
    lambda_sub = compute_lambda_sub(temp_C, sigma0)
    tau_sub = compute_tau_sub(temp_C, sigma0)
...
def pretty_axes(ax):
    """
    Apply final pretty touches to matplotlib axes for consistent styling.
    Args:
...
def plot_sigma0(temp_C, sigma0_interp_func):
    """
    Plot sigma0 as a function of temperature, including data points and linear fit.
    Args:
        temp_C (array-like): Temperatures in Celsius.
        sigma0_interp_func (callable): Interpolation function for sigma0.
    """
    ensure_plot_dir()
    temp_C = np.asarray(temp_C)
...
def plot_d0(temp_C, d0_values):
    """
    Plot capillary length d0 as a function of temperature.
    Args:
        temp_C (array-like): Temperatures in Celsius.
        d0_values (array-like): d0 values.
    """
    ensure_plot_dir()
    temp_C = np.asarray(temp_C)
...
def plot_beta_sub(temp_C, beta_sub_values):
    """
    Plot attachment coefficient beta_sub as a function of temperature (log scale).
    Args:
        temp_C (array-like): Temperatures in Celsius.
        beta_sub_values (array-like): beta_sub values.
    """
    ensure_plot_dir()
    temp_C = np.asarray(temp_C)
...
def plot_mobility(temp_C, M0_values):
    """
    Plot mobility M0 as a function of temperature (log scale).
    Args:
        temp_C (array-like): Temperatures in Celsius.
        M0_values (array-like): Mobility values.
    """
    ensure_plot_dir()
    temp_C = np.asarray(temp_C)
...
def plot_alpha_sub(temp_C, alpha_sub_values):
    """
    Plot attachment factor alpha_sub as a function of temperature (log scale).
    Args:
        temp_C (array-like): Temperatures in Celsius.
        alpha_sub_values (array-like): alpha_sub values.
    """
    ensure_plot_dir()
    temp_C = np.asarray(temp_C)
...
def main():
    """
    Main function to load data, compute physical parameters, and generate plots.
    """
