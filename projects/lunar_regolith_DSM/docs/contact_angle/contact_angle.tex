...
\newcommand{\sia}{\sigma_{ia}}
\newcommand{\sis}{\sigma_{is}}
\newcommand{\sas}{\sigma_{as}}
...
Following Moure \& Fu, the three \emph{interface energies} are $\sia$, $\sis$ and
$\sas$ [J\,m$^{-2}$]. The \emph{surface-tension parameters} built from them are
\begin{equation}
\Sigma_i=\sia+\sis-\sas,\quad
\Sigma_a=\sia+\sas-\sis,\quad
\Sigma_s=\sis+\sas-\sia .
\label{eq:Sigma}
\end{equation}
These combinations belong to the three-phase model, in which sediment is a bulk
phase field and each $\Sigma_k$ weights that phase's well. \textbf{They do not
apply here.} With the sediment moved to the wall there is no sediment phase: the
bulk well has a single ice--air interface, so
\begin{equation}
\Sigma_i=\Sigma_a=\sia ,
\end{equation}
and $\sis,\sas$ enter only through $f_w$. Reverting to \eqref{eq:Sigma} would
count the sediment twice, once in the well and once at the wall.

\emph{Known inconsistency in the shipped defaults, pre-dating this work:}
$\Sigma_i=0.109$ is $\sia$, a two-phase value, while $\Sigma_a=0.132$ is the
three-phase $\Sigma_a$ evaluated at the old sediment energies
($0.109+0.056-0.033$). For a two-phase model both should be $\sia$. $\Sigma_a$
feeds the double-well coefficient $C=(\Sigma_i+\Sigma_a)/2+\Lambda$, so it has
been setting the bulk interface energy to $0.1205$ rather than $0.109$ in every
lunar run. Left unchanged here because fixing it shifts bulk results across the
project.
...
\code{-sigma\_is} & ice--regolith interface energy [J\,m$^{-2}$] \\
\code{-sigma\_as} & air--regolith interface energy [J\,m$^{-2}$] \\
\code{-sigma\_ia} & ice--air interface energy; default \code{-Sigma\_i} \\
\bottomrule
...
$\sia$ defaults to \code{-Sigma\_i} because $\Sigma_i=\sia$ in this model
(\S1.1). Pass it explicitly whenever the substrate matters, rather than relying
on defaults whose provenance is mixed.
