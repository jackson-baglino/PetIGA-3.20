 ...
 * is not.
 *
 * Defaults (0, 0, Ly, 0) reproduce a flat floor at y=0 and a flat ceiling at
 * y=Ly exactly, so IC_COORD_WALLS degenerates to the plain rectangular
 * mapping for every geometry that sets no wall or bump options.
 *
...
/* Circular ice blob of radius Ly centred at (0.40 Lx, 0.5 Ly). */
static PetscReal ShapeIceSlab2D(PetscReal x, PetscReal y, const AppCtx *user)
{
    const PetscReal R    = user->Ly;
    const PetscReal dist = PetscSqrtReal(SQ(x - 0.40 * user->Lx)
                                       + SQ(y - 0.5  * user->Ly)) - R;

    return 0.5 - 0.5 * PetscTanhReal(0.5 * dist / user->eps);
}
 ...
 * packings. See -ic_grain_union in NASA_types.h for why this form is
 ...
 *
 * In a wedge the two walls are rays from the apex, and a circular arc centred
 * on that apex is perpendicular to every such ray -- so an apex-centred
 * annulus is exactly the shape that meets BOTH walls at 90 degrees, the
 * model's natural (Neumann) contact angle. Initialising anything else, a
 * circle in particular, starts the run with a contact-angle relaxation
 * transient that swamps the slow wedge-driven migration we want to measure.
 *
 * max(r1-rho, rho-r2) is the exact signed distance to the annulus (negative
 * inside), so the same tanh/tc used for every other feature gives the
 * equilibrium interface width here too. */
static PetscReal WedgeBandField(PetscReal x, PetscReal y, const AppCtx *user)
{
    const PetscReal tc  = PhaseSlope(user);
    PetscReal       ice = 0.0;

    for (PetscInt k = 0; k < user->n_wedge_bands; k++) {
        PetscReal rho = PetscSqrtReal(SQ(x - user->wedge_apex_x)
                                    + SQ(y - user->wedge_apex_y));
...
/* Ice plugs spanning a pore throat: the INTERSECTION of two meniscus discs
 * centred on the channel axis. Ice inside both discs makes each meniscus
 * CONVEX, i.e. the plug is barrel-shaped, wider at the axis than at its wall
 * contacts -- what a 90-degree contact angle forces in a converging-diverging
 * channel. (The hourglass of a wetting capillary bridge needs theta < 90,
 * which a Neumann wall BC cannot give.) An intersection of two discs is
 * already bounded, so it needs no lateral clipping and cannot leak ice down
 * channel.
 *
 * Sign consequence worth stating: convex menisci mean the TIGHTER throat holds
 * its ice at HIGHER curvature, hence higher equilibrium vapour density, making
 * it the vapour SOURCE. */
static PetscReal BridgeField(PetscReal x, PetscReal y, const AppCtx *user)
{
    const PetscReal tc  = PhaseSlope(user);
    PetscReal       ice = 0.0;
    ...
    for (PetscInt k = 0; k < user->n_bridges; k++) {
        ...
        PetscReal sdf = PetscMax(dL - user->bridge_rL[k],
                                 dR - user->bridge_rR[k]);
        ice += 0.5 - 0.5 * PetscTanhReal(tc * sdf);
    }
    return ice;
...
/* The multi-grain ice field: grains plus every optional extra body. */
static PetscReal ShapeMultiGrains2D(PetscReal x, PetscReal y, const AppCtx *user)
{
    ...
    ice += WedgeBandField(x, y, user);
    ice += BridgeField(x, y, user);
    return ice;
}
 ...
 * to an arbitrary number of grains. Optional extra ice bodies (shells, flats,
 * a wedge-bridging band, throat-spanning plugs) add on top; see the feature
 * fields above.
 *
...
PetscErrorCode FormInitialMultiGrains2D(IGA iga, Vec U, AppCtx *user)
{
    ...
    /* The wedge band is a standalone ice body, so a run may legitimately have
     * zero ice_grain_* circles. Only demand grains when nothing else would put
     * ice in the domain. */
    if (ng <= 0 && user->n_wedge_bands <= 0 && user->n_bridges <= 0)
