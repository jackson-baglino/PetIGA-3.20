#!/usr/bin/env python3
"""
plot_fields_highres.py — Convert PetIGA sol_*.dat output to VTK XML
structured-grid (.vts) files like plot_fields.py, but evaluate the TRUE
NURBS solution field at a dense set of points per element instead of writing
the raw control points.

Why: plot_fields.py writes nrb.points (the control net) directly as VTK
...
Usage
-----
  python plot_fields_highres.py                 # run from the output directory
  python plot_fields_highres.py --dir /path/to/run --n-per-elem 4
  python plot_fields_highres.py --dir /path/to/run --force
"""
...
def _vtk_coords_dense(C: np.ndarray) -> np.ndarray:
    """Same x-fastest VTK point reordering as plot_fields.py's
...
def convert(run_dir: str = ".", iga_file: str = "igasol.dat",
    ...
    if pvd_entries:
        pvd_path = os.path.join(run_dir, "pf_highres.pvd")
