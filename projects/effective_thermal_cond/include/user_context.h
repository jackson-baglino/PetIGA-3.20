...
typedef struct {
  ...
  /* Gauss-point bookkeeping (compact local indexing) */
  PetscInt ngp_local;     /* number of local Gauss points on this rank */
  PetscInt gp_cursor_sys; /* cursor during IGAComputeSystem() */
  PetscInt gp_cursor_scal;/* cursor during IGAComputeScalar() */
